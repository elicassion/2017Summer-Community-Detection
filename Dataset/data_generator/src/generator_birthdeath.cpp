...
int BirthDeathGenerator::mutate( int step, int num_nodes, deque<deque<int> > &member_matrix, multimap<int,int> &last_step_dynamic_map, multimap<int,int> &step_dynamic_map )
{
	...
	// Death events
	int actual_death = 0;
	for( int event = 0; event < num_death; event++)
	{
		// choose the cluster
		...
		for( cit = member_matrix[death_cluster_index].begin() ; cit != member_matrix[death_cluster_index].end(); cit++ )
		{
			...
			{
				// NB: don't count the dead community
				if( i != death_cluster_index && find(member_matrix[i].begin(), member_matrix[i].end(), node_index) != member_matrix[i].end() )
				{
		...
		Event death_event( step, "death" );
		death_event.add( dynamic_index+1 );
		events.push_back( death_event );
		actual_death++;
	}
	...
	for( int event = 0; event < num_birth; event++)
	{
		deque <int> first;
		...
		while( first.size() < target_size )
		{
			...
			else
			{
				...
				// already in the new community? don't add it again
				if( find(first.begin(), first.end(), node_index) != first.end() )
				{
					continue;
				}
...
#endif
		Event birth_event( step, "birth" );
		birth_event.add( dynamic_index + 1 );
		events.push_back( birth_event );
	}
