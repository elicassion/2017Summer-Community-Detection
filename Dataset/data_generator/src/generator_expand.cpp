...
int ExpandContractGenerator::mutate( int step, int num_nodes, deque<deque<int> > &member_matrix, multimap<int,int> &last_step_dynamic_map, multimap<int,int> &step_dynamic_map )
{
	...
	for( int event = 0; event < num_expansion; event++)
	{
		if( available.empty() )
		{
		...
		while( (int)member_matrix[expand_cluster_index].size() < target_size )		
		{
			...
			else
			{
				...
				// already in the expanding community? don't remove it 
				bool found_in_dest = find(member_matrix[expand_cluster_index].begin(), member_matrix[expand_cluster_index].end(), node_index) != member_matrix[expand_cluster_index].end();
				if( found_in_dest )
				{
					continue;
				}
...
#endif		
		Event expand_event( step, "expand" );
		expand_event.add( dynamic_index + 1 );
		events.push_back( expand_event );
		actual_expand++;
	}
	
	/// Contraction events
	int actual_contract = 0;
	for( int event = 0; event < num_contraction; event++)
	{
		if( available.empty() )
		{
		...
		{		
			...
			int node_index = member_matrix[contract_cluster_index][node_pos];
			// is the node really unassigned?
			bool assigned_elsewhere = false;
			for (int i = 0; i < member_matrix.size(); i++)
			{
				// NB: don't count the contracting community
				if( i != contract_cluster_index && find(member_matrix[i].begin(), member_matrix[i].end(), node_index) != member_matrix[i].end() )
				{
			...
			{
				unassigned.push_back(node_index);
			}
			member_matrix[contract_cluster_index].erase( member_matrix[contract_cluster_index].begin() + node_pos );
		}		
		Event contract_event( step, "contract" );
		contract_event.add( dynamic_index + 1 );
		events.push_back( contract_event );		
#ifdef DEBUG_MUTATION		
